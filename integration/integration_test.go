...
func TestCloneNonInteractive(t *testing.T) {
  ...
  "variables": [
    ...
    { "key": "USER_NAME", "value": "tester" },
    { "key": "USER_EMAIL", "value": "tester@example.com" },
    { "key": "VERSION", "value": "9.9.9" }
  ]
}`
    valsPath := writeFile(t, t.TempDir(), "values.json", vals)
...
func TestTemplateNonInteractive(t *testing.T) {
    ...
    cmd := exec.Command(bin, "clone",
        "--repo", "https://github.com/brasa-ai/template-tester.git",
        "--input", writeFile(t, t.TempDir(), "empty.json", `{"variables":[]}`),
        "--outputDir", work,
        "--startDelim", "[[", "--endDelim", "]]",
    )
    cmd.Dir = repoRoot(t)
