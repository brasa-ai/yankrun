...
func configPath() (string, error) {
	// Allow override via env var YANKRUN_CONFIG
	if p := os.Getenv("YANKRUN_CONFIG"); p != "" {
		return p, nil
	}
...
func Load() (*domain.Config, error) {
	...
	// Ensure parent dir exists only when using default location under home
	if os.Getenv("YANKRUN_CONFIG") == "" {
