...
func (t *TemplateAction) Execute(c *cli.Context) error {
	...
	counts, err := t.replacer.AnalyzeDir(dir, fileSizeLimit, startDelim, endDelim, cfg)
	if err != nil {
		return err
	}
	if len(counts) == 0 {
	...
	if err := t.replacer.ReplaceInDir(dir, final, cfg, fileSizeLimit, startDelim, endDelim, verbose); err != nil {
		return err
	}
